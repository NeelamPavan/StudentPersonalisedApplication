...
class OS:
    ...
    def main_window(self):
        ...
        process_btn = Button(self.root, text="Process Managemet", font="Helvetica 15 bold", width=20,
                             command=self.process_management)
        process_btn.place(relx=0.24, rely=0.23)

        Memory_btn = Button(self.root, text="Memory Managemet", font="Helvetica 15 bold", width=20,
                            command=self.memory_management)
        Memory_btn.place(relx=0.24, rely=0.37)

        schedule_btn = Button(self.root, text="Process Scheduling", font="Helvetica 15 bold", width=20,
                              command=self.process_scheduling)
        schedule_btn.place(relx=0.24, rely=0.53)

        PCB_btn = Button(self.root, text="Process Control Block", font="Helvetica 15 bold", width=20,
                         command=self.process_control_block)
    ...
    def process_management(self):
        ...
        destroy_btn = Button(self.root, text="Terminate a Process", font=FONT_BOLD, width=20,
        ...
        priority_btn = Button(self.root, text="Change the Priority", font=FONT_BOLD, width=20,
                              command=self.priority_change)
    ...
    def terminate_process(self):
        if len(processes) != 0:
            ...
            # Create the treeview
            tree = ttk.Treeview(frame, columns=("PID", "State", "Priority", "Pointer"), show="headings",
            ...
            for i in range(len(processes)):
                tree.insert("", END,
                            values=(processes[i].pid, processes[i].pstate, processes[i].priority, processes[i].point))
        ...
        if len(processes) != 0:
            ...
            for i in range(len(processes)):
                if processes[i].pstate == 'Block':
                    ...
                    processes[i].pid, processes[i].pstate, processes[i].priority, processes[i].point))
            if block == False:
                self.noblock_popup()
            # Add the treeview to the frame
            tree.pack(fill=BOTH, expand=YES)
    ...
    def rr(self):
        if len(processes) != 0:
            ...
            # Create the treeview
            tree = ttk.Treeview(frame, columns=("PID", "CT", "AT", "BT", "TT", "WT"), show="headings",
                                yscrollcommand=vscrollbar.set)
            tree.heading("PID", text="PID")
            ...
            for i in range(len(process)):
                tree.insert("", END, values=(
                process[i].pid, process[i].completion_time, process[i].arrival_time, process[i].burst_time,
