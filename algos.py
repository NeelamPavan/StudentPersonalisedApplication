...
def Enter():
    ...
    if string == 'Earliest DeadLine First':
        ...
        speedScale = Scale(master, from_=0.1, to=5.0, length=200, digits=2, resolution=0.2, orient=HORIZONTAL,
                           label="Delay in Sec [s]")
        speedScale.grid(row=size + 2, column=0, padx=5, pady=5)
        Button(master, text="Schedule", command=lambda: Schedule(master, speedScale.get()), bg='grey').grid(
            row=size + 3, column=0, padx=5, pady=5)
    ...
    if string == 'Rate Monotonic Scheduling':
        ...
        Button(master, text="Schedule", command=lambda: Schedule(master, speedScale.get()), bg='grey').grid(
            row=size + 2, column=0, padx=5, pady=5)
    ...
    if string == 'Round Robin':
        ...
        Label(frame, text="Arrival time", bg='#990033', foreground='white').grid(row=0, column=1, padx=5, pady=5,
                                                                                 sticky=W)
        for i in range(0, size):
            p = 'process', i + 1
            Label(frame, text=p, bg='#990033', foreground='black').grid(row=i + 1, column=0, padx=5, pady=5, sticky=W)
        ...
        Label(frame, text="Burst time", bg='#990033', foreground='white').grid(row=0, column=2, padx=5, pady=5,
                                                                               sticky=W)
        for i in range(0, size):
...
algMenu = ttk.Combobox(UI_frame, textvariable=selected_alg,
                       values=['Earliest DeadLine First', 'Rate Monotonic Scheduling', 'Deadline Monotonic Scheduling',
                               'Round Robin', 'Shortest Job First(premptive)', 'Shortest Job First(Non premptive)'])
